...
- 表格:轉成 markdown 表格,並嚴守下列規則。

表格規則(嚴格):
- **空白 cell 必須保留空**,以 `—` 標示;**禁止**用鄰列/鄰欄的值或任何推測值填補。
- **逐列逐欄對齊**:每個值確認落在正確的欄;某 cell 空白時,該列後續各值**不得左移錯位**。
- 看不清或不確定的 cell 標 `?`,不要猜。
- 不確定行列數時,寧可少填,**不要腦補撐滿**。
- 合併儲存格照原樣描述(不是空白,不用 `—`)。
...
- 不要寫「如圖所示」這種離開投影片就失效的指涉。
- 表格用 markdown,且嚴守:**空白 cell 保留空(用 `—`),禁止補值/推測;逐列逐欄對齊,空格不使後值左移;看不清標 `?`;不要腦補撐滿。** 合併儲存格照原樣描述。
